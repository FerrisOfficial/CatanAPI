...
class HexPackTest : public ::testing::TestWithParam<
                        std::tuple<uint8_t, Resource, uint8_t, uint8_t>> {};

TEST_P(HexPackTest, HexPacking) {
    auto [catanNumber, resource, player0Value, player1Value] = GetParam();

    Hex::PackedHex hex =
        Hex::makeHex(catanNumber, resource, player0Value, player1Value);
...
INSTANTIATE_TEST_SUITE_P(
    HexPackTests, HexPackTest,
    ::testing::Values(std::make_tuple(6, Resource::Brick, 3, 5),
                      ...
                      std::make_tuple(0, Resource::NoResource, 0,
                                      0)  // Min values
                      ));

class NodePackTest : public ::testing::TestWithParam<
                         std::tuple<HexId, HexId, HexId, EdgeId, EdgeId, EdgeId,
                                    StructureType, PlayerId, PortType>> {};

TEST_P(NodePackTest, NodePacking) {
    auto [hex1, hex2, hex3, edge1, edge2, edge3, structure, owner, portType] =
        GetParam();

    Node::PackedNode node = Node::makeNode(hex1, hex2, hex3, edge1, edge2,
                                           edge3, structure, owner, portType);
...
INSTANTIATE_TEST_SUITE_P(
    NodePackTests, NodePackTest,
    ::testing::Values(
        std::make_tuple(4, 5, 9, 20, 27, 28, StructureType::Settlement,
                        PlayerId::Player0, PortType::WoolPort),
        std::make_tuple(5, 6, 10, 21, 29, 30, StructureType::City,
                        PlayerId::Player1, PortType::GrainPort),
        std::make_tuple(0, 0, 0, 0, 0, 0, StructureType::NoStructure,
                        PlayerId::Player0, PortType::NoPort),
        std::make_tuple(7, 12, HexIdNone, 39, 40, 49, StructureType::Road,
                        PlayerId::Player1,
                        PortType::OrePort),  // Max hex values
        std::make_tuple(15, HexIdNone, HexIdNone, 53, 61, EdgeIdNone,
                        StructureType::City, PlayerId::Player0,
                        PortType::LumberPort)));

class EdgePackTest : public ::testing::TestWithParam<
                         std::tuple<bool, PlayerId, NodeId, NodeId>> {};
...
INSTANTIATE_TEST_SUITE_P(
    EdgePackTests, EdgePackTest,
    ::testing::Values(
        std::make_tuple(true, PlayerId::Player0, 25, 24),
        std::make_tuple(false, PlayerId::Player1, 5, 6),
        std::make_tuple(true, PlayerId::Player1, 53,
                        52),  // Max node values within range
...
TEST(HexTest, IndividualFieldPacking) {
    ...
    EXPECT_EQ(Hex::unpackCatanNumber(hex),
              8);  // Should not affect other fields
...
TEST(BoardTest, ConstantValidation) {
    ...
    EXPECT_EQ(NODE_COUNT,
              54);  // Standard Catan board has 54 intersection points
...
TEST(BoardTest, BitBoundaries) {
    ...
    // Node boundaries
    Node::PackedNode node = 0;
    node = Node::packAdjacentHex(
        node, 0, 31);  // 5 bits max (19 is actual max for hex count)
    EXPECT_EQ(Node::unpackAdjacentHex(node, 0), 31);

    // Edge boundaries
    Edge::PackedEdge edge = 0;
    edge = Edge::packAdjacentNode(
        edge, 0, 127);  // 7 bits max (54 is actual max for node count)
