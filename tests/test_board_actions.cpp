...
TEST_F(ApplyActionTest, ExpectPlaceInitialStructures) {
    ...
    boardState.packedPlayers[static_cast<size_t>(playerId)] =
        Player::packVictoryPoints(
            boardState.packedPlayers[static_cast<size_t>(playerId)], 0);
    boardState.packedPlayers[static_cast<size_t>(playerId)] =
    ...
    placeInitialStructuresAction = Action::packType(
        placeInitialStructuresAction, ActionType::PlaceInitialStructures);
    placeInitialStructuresAction =
        Action::packPlayerID(placeInitialStructuresAction, playerId);
    placeInitialStructuresAction =
        Action::packArg1(placeInitialStructuresAction, settlementNodeId);
    placeInitialStructuresAction =
        Action::packArg2(placeInitialStructuresAction, roadEdgeId);

    boardState.applyAction(placeInitialStructuresAction);

    EXPECT_EQ(Node::unpackStructure(boardState.nodes[settlementNodeId]),
              StructureType::Settlement);
    EXPECT_EQ(Node::unpackOwner(boardState.nodes[settlementNodeId]), playerId);
    EXPECT_EQ(Player::unpackVictoryPoints(
                  boardState.packedPlayers[static_cast<size_t>(playerId)]),
              1);
...
TEST_F(ApplyActionTest, ExpextPlace2InitialSettlement) {
    ...
    HexId adjHex[3] = {Node::unpackAdjacentHex(settlementNode, 0),
    ...
    for (HexId h : adjHex) {
        if (h != HexIdNone) {
            boardState.hexes[h] =
                Hex::packResource(boardState.hexes[h], Resource::Brick);
        }
    }
    ...
    place2SettlementAction = Action::packType(
        place2SettlementAction, ActionType::Place2InitialStructures);
    place2SettlementAction =
        Action::packPlayerID(place2SettlementAction, playerId);
    place2SettlementAction =
        Action::packArg1(place2SettlementAction, settlementNodeId);
    ...
    EXPECT_EQ(Player::unpackResource(
                  boardState.packedPlayers[static_cast<size_t>(playerId)],
                  Resource::Brick),
              9);
    ...
    for (HexId h : adjHex) {
        if (h != HexIdNone) {
            EXPECT_EQ(Hex::unpackPlayerValue(boardState.hexes[h], playerId), 1);
        }
    }
}
...
TEST_F(ApplyActionTest, ExpectResourceDistributionOnEightRoll) {
    ...
    boardState.hexes[hex1] =
        Hex::packPlayerValue(boardState.hexes[hex1], PlayerId::Player0, 3);
    boardState.hexes[hex1] =
        Hex::packPlayerValue(boardState.hexes[hex1], PlayerId::Player1, 2);
    boardState.hexes[hex2] =
        Hex::packPlayerValue(boardState.hexes[hex2], PlayerId::Player0, 4);
    boardState.hexes[hex2] =
        Hex::packPlayerValue(boardState.hexes[hex2], PlayerId::Player1, 0);
    ...
    auto player0Resources1 =
        Player::unpackResource(boardState.packedPlayers[0], res1);
    auto player0Resources2 =
        Player::unpackResource(boardState.packedPlayers[0], res2);
    auto player1Resources1 =
        Player::unpackResource(boardState.packedPlayers[1], res1);
    auto player1Resources2 =
        Player::unpackResource(boardState.packedPlayers[1], res2);
...
TEST_F(ApplyActionTest, ExpectTurnRotationOnEndTurn) {
    ...
    PlayerId expectedNextPlayer = (startingPlayer == PlayerId::Player0)
                                      ? PlayerId::Player1
                                      : PlayerId::Player0;
...
TEST_F(ApplyActionTest, ExpectRobberMovementAndStealCardTrigger) {
    EXPECT_EQ(Hex::unpackResource(boardState.hexes[boardState.robberPosition]),
              Resource::NoResource);
    ...
    boardState.hexes[newRobberPosition] = Hex::packPlayerValue(
        boardState.hexes[newRobberPosition], victimPlayer, 2);

    Action::PackedAction moveRobberAction{};
    moveRobberAction =
        Action::packType(moveRobberAction, ActionType::MoveRobber);
    ...
    boardState.applyAction(Action::packArg1(
        Action::packPlayerID(Action::packType(0, ActionType::StealResource),
                             stealingPlayer),
...
TEST_F(ApplyActionTest, ExpectStealResource) {
    ...
    stealResourceAction =
        Action::packType(stealResourceAction, ActionType::StealResource);
    stealResourceAction = Action::packArg1(
        stealResourceAction, static_cast<uint8_t>(robbedResource));
    stealResourceAction =
        Action::packPlayerID(stealResourceAction, stealingPlayer);
...
TEST_F(ApplyActionTest, ExpectDiscardResources) {
    ...
    discardResourcesAction =
        Action::packType(discardResourcesAction, ActionType::DiscardResources);
    discardResourcesAction =
        Action::packPlayerID(discardResourcesAction, discardingPlayer);
    discardResourcesAction =
        Action::packResource(discardResourcesAction, Resource::Ore, 1);
    discardResourcesAction =
        Action::packResource(discardResourcesAction, Resource::Wool, 2);
    discardResourcesAction =
        Action::packResource(discardResourcesAction, Resource::Grain, 3);
    discardResourcesAction =
        Action::packResource(discardResourcesAction, Resource::Brick, 0);
    discardResourcesAction =
        Action::packResource(discardResourcesAction, Resource::Lumber, 1);
...
TEST_F(ApplyActionTest, ExpectBuildRoad) {
    ...
    EXPECT_EQ(Bank::unpackResource(boardState.packedBank, Resource::Lumber),
              11);
    EXPECT_EQ(Player::unpackResource(
    ...
    EXPECT_EQ(Player::unpackAvailableStructures(
              ...
              14);
    EXPECT_EQ(
        Player::unpackLongestRoadLength(
            boardState.packedPlayers[static_cast<size_t>(buildingPlayer)]),
        1);
    EXPECT_FALSE(Player::unpackLongestRoadFlag(
        boardState.packedPlayers[static_cast<size_t>(buildingPlayer)]));
}
...
TEST_F(ApplyActionTest, ExpectBuildRoadGetLongestRoadAward) {
    ...
    for (EdgeId e = 0; e < 4; ++e) {
        Action::PackedAction buildRoadAction{};
        buildRoadAction =
            Action::packType(buildRoadAction, ActionType::BuildRoad);
    ...
    buildAnotherRoadAction =
        Action::packType(buildAnotherRoadAction, ActionType::BuildRoad);
    buildAnotherRoadAction = Action::packArg1(buildAnotherRoadAction, 21);
    buildAnotherRoadAction =
        Action::packPlayerID(buildAnotherRoadAction, buildingPlayer);
    ...
    EXPECT_EQ(
        Player::unpackLongestRoadLength(
            boardState.packedPlayers[static_cast<size_t>(buildingPlayer)]),
        5);
    EXPECT_TRUE(Player::unpackLongestRoadFlag(
        boardState.packedPlayers[static_cast<size_t>(buildingPlayer)]));
}
...
TEST_F(ApplyActionTest, ExpectBuildRoadNotGetLongestRoadAwardDueToTie) {
    ...
    EXPECT_EQ(
        Player::unpackLongestRoadLength(
            boardState.packedPlayers[static_cast<size_t>(buildingPlayer)]),
        5);
    EXPECT_EQ(Player::unpackLongestRoadLength(
                  boardState.packedPlayers[static_cast<size_t>(enemyPlayer)]),
              5);
    EXPECT_TRUE(Player::unpackLongestRoadFlag(
        boardState.packedPlayers[static_cast<size_t>(buildingPlayer)]));
    EXPECT_FALSE(Player::unpackLongestRoadFlag(
...
TEST_F(ApplyActionTest, ExpectNewPlayerGetLongestRoadAward) {
    ...
    EXPECT_EQ(Player::unpackLongestRoadLength(
                  boardState.packedPlayers[static_cast<size_t>(firstPlayer)]),
              5);
    EXPECT_TRUE(Player::unpackLongestRoadFlag(
        boardState.packedPlayers[static_cast<size_t>(firstPlayer)]));
    EXPECT_FALSE(Player::unpackLongestRoadFlag(
        boardState.packedPlayers[static_cast<size_t>(secondPlayer)]));
...
TEST_F(ApplyActionTest, ExpectUndoBuildRoadUpdateLongestRoad) {
    ...
    EXPECT_TRUE(Player::unpackLongestRoadFlag(
        boardState.packedPlayers[static_cast<size_t>(buildingPlayer)]));
    EXPECT_FALSE(Player::unpackLongestRoadFlag(
        boardState.packedPlayers[static_cast<size_t>(PlayerId::Player1)]));
...
TEST_F(ApplyActionTest, ExpectLoseLongestRoadFlagDueToEnemySettlement) {
    ...
    buildSettlementAction =
        Action::packType(buildSettlementAction, ActionType::BuildSettlement);
    buildSettlementAction = Action::packArg1(
        buildSettlementAction, 19);  // Node in the middle of the road
    buildSettlementAction =
        Action::packPlayerID(buildSettlementAction, enemyPlayer);
...
TEST_F(ApplyActionTest, ExpectBuildSettlement) {
    ...
    buildSettlementAction =
        Action::packType(buildSettlementAction, ActionType::BuildSettlement);
    buildSettlementAction =
        Action::packArg1(buildSettlementAction, settlementNodeId);
    buildSettlementAction =
        Action::packPlayerID(buildSettlementAction, buildingPlayer);
    boardState.applyAction(buildSettlementAction);
    ...
    for (HexId h : adjHex) {
        if (h != HexIdNone) {
            EXPECT_EQ(
                Hex::unpackPlayerValue(boardState.hexes[h], buildingPlayer), 1);
        }
    }
}
...
TEST_F(ApplyActionTest, ExpectBuildCity) {
    ...
    boardState.packedPlayers[static_cast<size_t>(buildingPlayer)] =
        Player::packAvailableStructures(
            boardState.packedPlayers[static_cast<size_t>(buildingPlayer)],
            StructureType::Settlement, 3);
    boardState.packedPlayers[static_cast<size_t>(buildingPlayer)] =
        Player::packVictoryPoints(
            boardState.packedPlayers[static_cast<size_t>(buildingPlayer)], 1);
    ...
    HexId adjHex[3] = {Node::unpackAdjacentHex(cityNode, 0),
    ...
    for (HexId h : adjHex) {
        if (h != HexIdNone) {
            boardState.hexes[h] =
                Hex::packPlayerValue(boardState.hexes[h], buildingPlayer, 1);
        }
    }
    ...
    for (HexId h : adjHex) {
        if (h != HexIdNone) {
            EXPECT_EQ(
                Hex::unpackPlayerValue(boardState.hexes[h], buildingPlayer), 2);
        }
    }
}
...
TEST_P(ActionTestDevCardTypeParam, ExpectBuyOneDevelopmentCard) {
    ...
    // Ensure the bank contains only one type of development cards for this
    // test.
    for (DevType devType :
        ...
        if (devType == devCardInBank) {
            boardState.packedBank =
                Bank::packDevCard(boardState.packedBank, devType, 3);
        } else {
            boardState.packedBank =
    ...
    buyDevCardAction =
        Action::packType(buyDevCardAction, ActionType::BuyDevCard);
    ...
    EXPECT_EQ(Player::unpackResource(
                  boardState.packedPlayers[static_cast<size_t>(buyingPlayer)],
                  Resource::Brick),
              7);
    EXPECT_EQ(Player::unpackResource(
                  boardState.packedPlayers[static_cast<size_t>(buyingPlayer)],
    ...
    int totalDevCards = Player::totalDevCards(
        boardState.packedPlayers[static_cast<size_t>(buyingPlayer)]);

    EXPECT_EQ(totalDevCards, 1);

    if (Player::unpackDevCard(
            boardState.packedPlayers[static_cast<size_t>(buyingPlayer)],
            DevType::VictoryPoint) == 1) {
        EXPECT_EQ(
            Player::unpackVictoryPoints(
                boardState.packedPlayers[static_cast<size_t>(buyingPlayer)]),
            1);
...
INSTANTIATE_TEST_SUITE_P(DevCardTypes, ActionTestDevCardTypeParam,
                         ::testing::Values(DevType::Knight,
...
TEST_F(ApplyActionTest, ExpectBuyAllDevCards) {
    ...
    for (DevType devType :
         {DevType::Knight, DevType::RoadBuilding, DevType::YearOfPlenty,
          DevType::Monopoly, DevType::VictoryPoint}) {
        boardState.packedBank =
            Bank::packDevCard(boardState.packedBank, devType, 1);
    }
    ...
    EXPECT_EQ(Player::unpackVictoryPoints(
                  boardState.packedPlayers[static_cast<size_t>(buyingPlayer)]),
              1);
    EXPECT_EQ(Player::totalDevCards(
                  boardState.packedPlayers[static_cast<size_t>(buyingPlayer)]),
              5);
}
...
TEST_F(ApplyActionTest, ExpectTradeBank) {
    ...
    tradeBankAction =
        Action::packArg1(tradeBankAction, static_cast<uint8_t>(giveResource));
    tradeBankAction = Action::packArg2(tradeBankAction,
    ...
    EXPECT_EQ(Player::unpackResource(
                  boardState.packedPlayers[static_cast<size_t>(tradingPlayer)],
                  giveResource),
              5);
    EXPECT_EQ(Player::unpackResource(
                  boardState.packedPlayers[static_cast<size_t>(tradingPlayer)],
...
TEST_F(ApplyActionTest, ExpectReceiveResources) {
    ...
    receiveResourcesAction =
        Action::packType(receiveResourcesAction, ActionType::ReceiveResources);
    receiveResourcesAction = Action::packArg1(
        receiveResourcesAction, static_cast<uint8_t>(resToReceive));
    receiveResourcesAction = Action::packArg2(receiveResourcesAction, 4);
    receiveResourcesAction =
        Action::packPlayerID(receiveResourcesAction, receivingPlayer);
    boardState.applyAction(receiveResourcesAction);

    EXPECT_EQ(Bank::unpackResource(boardState.packedBank, resToReceive), 6);
    EXPECT_EQ(
        Player::unpackResource(
            boardState.packedPlayers[static_cast<size_t>(receivingPlayer)],
            resToReceive),
        4);
}
...
TEST_F(ApplyActionTest, ExpectPlayDevCardKnight) {
    ...
    playKnightAction =
        Action::packType(playKnightAction, ActionType::PlayDevCardKnight);
    ...
    EXPECT_EQ(boardState.robberPosition, newRobberPosition);
    EXPECT_EQ(Player::unpackDevCard(
                  boardState.packedPlayers[static_cast<size_t>(playingPlayer)],
                  DevType::Knight),
              0);
    EXPECT_EQ(Player::unpackUsedKnights(
                  boardState.packedPlayers[static_cast<size_t>(playingPlayer)]),
              1);
}
...
TEST_F(ApplyActionTest, ExpectLargestArmyFlag) {
    ...
    boardState.packedPlayers[static_cast<size_t>(playerA)] =
        Player::packUsedKnights(
            boardState.packedPlayers[static_cast<size_t>(playerA)], 2);
    boardState.packedPlayers[static_cast<size_t>(playerB)] =
        Player::packUsedKnights(
            boardState.packedPlayers[static_cast<size_t>(playerB)], 2);
    ...
    EXPECT_TRUE(Player::unpackLargestArmyFlag(
        boardState.packedPlayers[static_cast<size_t>(playerB)]));
}
...
TEST_F(ApplyActionTest, ExpectChangeLargestArmyFlag) {
    ...
    boardState.packedPlayers[static_cast<size_t>(playerA)] =
        Player::packUsedKnights(
            boardState.packedPlayers[static_cast<size_t>(playerA)], 3);
    boardState.packedPlayers[static_cast<size_t>(playerA)] =
        Player::packLargestArmyFlag(
            boardState.packedPlayers[static_cast<size_t>(playerA)], true);
    ...
    playKnightAction1 =
        Action::packType(playKnightAction1, ActionType::PlayDevCardKnight);
    ...
    playKnightAction2 =
        Action::packType(playKnightAction2, ActionType::PlayDevCardKnight);
    ...
    EXPECT_TRUE(Player::unpackLargestArmyFlag(
        boardState.packedPlayers[static_cast<size_t>(playerB)]));
    EXPECT_FALSE(Player::unpackLargestArmyFlag(
        boardState.packedPlayers[static_cast<size_t>(playerA)]));
}
...
TEST_F(ApplyActionTest, ExpectPlayDevCardRoadBuilding) {
    ...
    playRoadBuildingAction = Action::packType(
        playRoadBuildingAction, ActionType::PlayDevCardRoadBuilding);
    playRoadBuildingAction =
        Action::packArg1(playRoadBuildingAction, firstRoadEdgeId);
    playRoadBuildingAction =
        Action::packArg2(playRoadBuildingAction, secondRoadEdgeId);
    playRoadBuildingAction =
        Action::packPlayerID(playRoadBuildingAction, playingPlayer);
...
TEST_F(ApplyActionTest, ExpectPlayDevCardYearOfPlenty) {
    ...
    playYearOfPlentyAction = Action::packType(
        playYearOfPlentyAction, ActionType::PlayDevCardYearOfPlenty);
    playYearOfPlentyAction = Action::packArg1(
        playYearOfPlentyAction, static_cast<uint8_t>(firstResource));
    playYearOfPlentyAction = Action::packArg2(
        playYearOfPlentyAction, static_cast<uint8_t>(secondResource));
    playYearOfPlentyAction =
        Action::packPlayerID(playYearOfPlentyAction, playingPlayer);
...
TEST_F(ApplyActionTest, ExpectPlayDevCardMonopoly) {
    ...
    playMonopolyAction =
        Action::packType(playMonopolyAction, ActionType::PlayDevCardMonopoly);
    playMonopolyAction = Action::packArg1(
        playMonopolyAction, static_cast<uint8_t>(monopolyResource));
    playMonopolyAction =
        Action::packPlayerID(playMonopolyAction, playingPlayer);
