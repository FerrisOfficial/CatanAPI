...
TEST(GameTest, GameBoardHasCorrectNodeCount) {
    ...
    EXPECT_EQ(Board::Node::unpackAdjacentHex(player1.boardState->nodes[0], 1), 0);
    
    // Node 10 should connect to hexes: 0, 1, 4
    EXPECT_EQ(Board::Node::unpackAdjacentHex(player1.boardState->nodes[10], 0), 0);
    EXPECT_EQ(Board::Node::unpackAdjacentHex(player1.boardState->nodes[10], 1), 1);
    EXPECT_EQ(Board::Node::unpackAdjacentHex(player1.boardState->nodes[10], 2), 4);
}
...
TEST(GameTest, GameBoardHasCorrectEdgeCount) {
    ...
    EXPECT_EQ(Board::Edge::unpackAdjacentNode(player1.boardState->edges[0], 0), 0);
    EXPECT_EQ(Board::Edge::unpackAdjacentNode(player1.boardState->edges[0], 1), 1);
    
    // Edge 12 connects nodes 9 and 10
    EXPECT_EQ(Board::Edge::unpackAdjacentNode(player1.boardState->edges[12], 0), 9);
    EXPECT_EQ(Board::Edge::unpackAdjacentNode(player1.boardState->edges[12], 1), 10);
}
