...
PortType port_for_resource(Resource r) {
    switch (r) {
        case Resource::Brick:
            return PortType::BrickPort;
        case Resource::Lumber:
            return PortType::LumberPort;
        case Resource::Wool:
            return PortType::WoolPort;
        case Resource::Grain:
            return PortType::GrainPort;
        case Resource::Ore:
            return PortType::OrePort;
        default:
            return PortType::NoPort;
    }
}

int node_resource_pips(const Board::BoardState* board, NodeId nodeId,
                       Resource r) {
...
bool node_has_2to1_port(const Board::BoardState* board, NodeId nodeId,
                        Resource r) {
...
Resource choose_priority_resource(const Board::BoardState* board) {
    ...
    // - maxNodePips: best single node (how many pips of this resource one
    ...
    for (Resource r :
        ...
        // - strong single node (e.g., 3 hexes of this resource) is very
        // important,
        // - sum of pips and number of hexes somewhat boost the score.
        int s = 0;
        s += 50 *
             totalPips[idx];  // general "presence" of the resource on the map
        s += 40 *
             maxNodePips[idx];  // how strong can the best settlement spot be
        s += 60 *
             bestPortNodePips[idx];  // how strong can the spot with 2:1 port be
        s += 10 * hexCount[idx];     // prefer resource with many hexes

        // Minimal tie-breaker: slightly reward brick/lumber because they
        // provide roads
...
PlacementScore pick_best_initial_port(
    const Board::BoardState* board,
    const std::vector<Action::PackedAction>& actions, Resource prio) {
    PlacementScore best;
    for (const auto a : actions) {
        ...
        if (!node_has_2to1_port(board, nodeId, prio))
            continue;  // enforce 2:1 for first
        ...
        // Slight bonus if the road leads to a node with many adjacent edges
        // (future options).
...
PlacementScore pick_best_second_placement(
    const Board::BoardState* board,
    const std::vector<Action::PackedAction>& actions, Resource prio,
    ...
    for (const auto a : actions) {
        ...
        for (size_t i = 0; i < 5; ++i) {
            if (has[i] && !firstRes[i] && i != static_cast<size_t>(prio))
                s += 10;
        }
...
int road_priority_potential(const Board::BoardState* board, EdgeId edgeId,
                            Resource prio) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        const auto adjEdges =
            Board::Node::getAdjacentEdges(board->nodes[node1]);
...
OneResourcePlayer::getInitialPlacement() {
    auto actions =
    ...
    // Try to pick an action that places on a 2:1 port for the prioritized
    // resource.
    auto best =
        pick_best_initial_port(boardState, actions, prioritizedResource);

    // If none found due to legal constraints, try any resource that has a 2:1
    // among actions.
...
std::pair<Action::PackedAction, Action::PackedAction>
OneResourcePlayer::get2InitialPlacement() {
    auto actions =
    ...
    auto best = pick_best_second_placement(
        boardState, actions, prioritizedResource, firstPlacementResources);
    return {best.action, best.action};
}

// Helper copied conceptually from It5: pick target build minimizing deficit
// with tie-breaks
...
Action::PackedAction OneResourcePlayer::getTurnAction() {
    ...
    if (!cityActions.empty()) {
        for (const auto a : cityActions) {
            const NodeId nodeId = Action::unpackArg1(a);
            const int prioPips =
                node_resource_pips(boardState, nodeId, prioritizedResource);
    ...
    // 3) TRADE – use accumulated prioritized resource to get other resources
    // via 2:1 port. Prefer trades that SPEND prioritized resource (we should
    // have a lot of it) to get resources needed for building.
    if (!tradeActions.empty()) {
        using PlayerHelpers::cost_for;
        using PlayerHelpers::deficit;

        const auto preHave = unpack_resources(
            boardState->packedPlayers[static_cast<uint8_t>(selfId)]);

        for (const auto a : tradeActions) {
            boardState->applyAction(a);
            const auto postHave = unpack_resources(
                boardState->packedPlayers[static_cast<uint8_t>(selfId)]);
            boardState->undoLastAction();

            const int delta =
                static_cast<int>(
                    postHave[static_cast<size_t>(prioritizedResource)]) -
                static_cast<int>(
                    preHave[static_cast<size_t>(prioritizedResource)]);

            // We want to SPEND the prioritized resource (delta < 0) to get
            // other resources
            if (delta >= 0) {
                continue;
            }
            ...
            const auto cityDefPre =
                deficit(preHave, cost_for(BuyableType::City));
            const auto cityDefPost =
                deficit(postHave, cost_for(BuyableType::City));
            const auto settleDefPre =
                deficit(preHave, cost_for(BuyableType::Settlement));
            const auto settleDefPost =
                deficit(postHave, cost_for(BuyableType::Settlement));
            ...
            // Extra bonus for spending prioritized resource efficiently (via
            // 2:1 port)
            bonus += 200 * (-delta);
    ...
    // 4) ROADS – those that potentially lead to fields with prioritized
    // resource / 2:1 port.
    if (!roadActions.empty()) {
        for (const auto a : roadActions) {
            const EdgeId e = Action::unpackArg1(a);
            const int bonus =
                road_priority_potential(boardState, e, prioritizedResource);
            int s = simulate_score(a, bonus);
            if (s > bestScore) {
                bestScore = s;
                best = a;
