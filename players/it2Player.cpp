...
namespace {
Action::PackedAction pick_random(const std::vector<Action::PackedAction>& pool) {
    if (pool.empty()) {
...
bool is_player_port_node(const Board::BoardState* board, PlayerId playerId, NodeId nodeId) {
    ...
    return owner == playerId && (structure == StructureType::Settlement || structure == StructureType::City);
}
...
Ports detect_ports(const Board::BoardState* board, PlayerId playerId) {
    ...
    for (NodeId nodeId : brickPortsNodes) {
        if (is_player_port_node(board, playerId, nodeId)) { p.twoForOneByResource[static_cast<size_t>(Resource::Brick)] = true; break; }
    }
    for (NodeId nodeId : lumberPortsNodes) {
        if (is_player_port_node(board, playerId, nodeId)) { p.twoForOneByResource[static_cast<size_t>(Resource::Lumber)] = true; break; }
    }
    for (NodeId nodeId : woolPortsNodes) {
        if (is_player_port_node(board, playerId, nodeId)) { p.twoForOneByResource[static_cast<size_t>(Resource::Wool)] = true; break; }
    }
    for (NodeId nodeId : grainPortsNodes) {
        if (is_player_port_node(board, playerId, nodeId)) { p.twoForOneByResource[static_cast<size_t>(Resource::Grain)] = true; break; }
    }
    for (NodeId nodeId : orePortsNodes) {
        if (is_player_port_node(board, playerId, nodeId)) { p.twoForOneByResource[static_cast<size_t>(Resource::Ore)] = true; break; }
    }

    // 3:1 port (generic)
    for (NodeId nodeId : threeForOnePortsNodes) {
        if (is_player_port_node(board, playerId, nodeId)) { p.threeForOne = true; break; }
    }

    return p;
}

Action::PackedAction build_trade(PlayerId playerId, Resource give, Resource receive, uint8_t ratio) {
...
uint16_t total_deficit(const std::array<uint8_t, 5>& have, const std::array<uint8_t, 5>& need) {
    uint16_t sum = 0;
    for (size_t i = 0; i < 5; ++i) {
        if (have[i] < need[i]) sum = static_cast<uint16_t>(sum + (need[i] - have[i]));
    }
    return sum;
}

struct Closeness {
    uint16_t remainingMissing = 0;  // cards still missing after best-case trading
    uint16_t maxTradeableUnits = 0; // how many missing cards we could potentially acquire via trades
...
Closeness evaluate_closeness(const std::array<uint8_t, 5>& have, const std::array<uint8_t, 5>& need, const Ports& ports) {
    Closeness c;

    uint16_t missing = 0;
    for (size_t i = 0; i < 5; ++i) {
        if (have[i] < need[i]) missing = static_cast<uint16_t>(missing + (need[i] - have[i]));
    }
    c.rawMissing = missing;

    uint16_t tradeable = 0;
    for (size_t i = 0; i < 5; ++i) {
        const uint8_t surplus = have[i] > need[i] ? static_cast<uint8_t>(have[i] - need[i]) : 0;
    ...
    c.remainingMissing = (missing > tradeable) ? static_cast<uint16_t>(missing - tradeable) : 0;
    return c;
}

int priority_index(BuyableType b) {
    // Value tie-break for "equally close" targets.
    // Keep consistent with the original higher-level preference (City > Settlement > Road > Dev).
    switch (b) {
...
BuyableType choose_closest_target(const std::array<uint8_t, 5>& have, const Ports& ports) {
    BuyableType bestType = BuyableType::Road;
    Closeness best = {std::numeric_limits<uint16_t>::max(), 0, std::numeric_limits<uint16_t>::max()};
    ...
    for (BuyableType t : {BuyableType::City, BuyableType::Settlement, BuyableType::Road, BuyableType::DevCard}) {
        ...
        // Secondary: if equally reachable after trades, prefer higher-value buyables.
...
Action::PackedAction choose_trade_for_target(
    const std::array<uint8_t, 5>& have,
    ...
    const auto try_ratio = [&](uint8_t ratio, bool (*ratio_allowed)(const Ports&, size_t)) {
        for (size_t recv = 0; recv < 5; ++recv) {
            if (have[recv] >= need[recv]) continue; // not missing this resource

            for (size_t give = 0; give < 5; ++give) {
                if (give == recv) continue;
                if (!ratio_allowed(ports, give)) continue;

                // Simple rule: only spend from surplus relative to the target cost.
                const uint8_t surplus = have[give] > need[give] ? static_cast<uint8_t>(have[give] - need[give]) : 0;
                ...
                const uint16_t improvement = static_cast<uint16_t>(beforeDef - afterDef);
                // Prefer the trade that gets us closest to affording the target.
                if (afterDef < bestAfterDef || (afterDef == bestAfterDef && improvement > bestImprovement)) {
                    bestAfterDef = afterDef;
                    bestImprovement = improvement;
                    best = build_trade(playerId, static_cast<Resource>(give), static_cast<Resource>(recv), ratio);
                }
            }
        }
    };
    ...
    try_ratio(2, [](const Ports& p, size_t give) { return p.twoForOneByResource[give]; });
    if (bestImprovement > 0) {
        if (std::find(legalTrades.begin(), legalTrades.end(), best) == legalTrades.end()) {
            throw std::runtime_error("It2Player chose a 2:1 trade that is not in legal actions");
        }
        return best;
    }
    ...
    if (bestImprovement > 0) {
        if (std::find(legalTrades.begin(), legalTrades.end(), best) == legalTrades.end()) {
            throw std::runtime_error("It2Player chose a 3:1 trade that is not in legal actions");
        }
        return best;
    }
    ...
    if (bestImprovement > 0) {
        if (std::find(legalTrades.begin(), legalTrades.end(), best) == legalTrades.end()) {
            throw std::runtime_error("It2Player chose a 4:1 trade that is not in legal actions");
        }
    }
    return best;
}
...
Action::PackedAction It2Player::getTurnAction() {
    ...
    // Try to trade towards the closest purchase (after best-case trading), even if
    // we could currently build a lower-priority thing (road/dev).
    if (!tradeActions.empty()) {
        const auto playerId = boardState->currentPlayer;
        const auto ports = detect_ports(boardState, playerId);
        const auto packed = boardState->packedPlayers[static_cast<uint8_t>(playerId)];
        const auto have = unpack_resources(packed);

        // Choose what we're trying to buy based on how close we are (after best-case trading).
        const BuyableType target = choose_closest_target(have, ports);
        const auto need = cost_for(target);

        auto trade = choose_trade_for_target(have, need, ports, playerId, tradeActions);
        if (Action::unpackType(trade) == ActionType::TradeBank) {
            return trade;
        }
    }

    // No helpful trade selected: follow It1-style build preference for remaining types.
    if (!roadActions.empty()) return pick_random(roadActions);
    if (!devActions.empty()) return pick_random(devActions);

    // Otherwise keep behaving like Random: random among non-EndTurn actions first, then EndTurn.
