...
RandomPlayer::getInitialPlacement() {
    auto actions =
...
std::pair<Action::PackedAction, Action::PackedAction>
RandomPlayer::get2InitialPlacement() {
    auto actions =
...
Action::PackedAction RandomPlayer::getDevAction() {
    auto actions =
        boardState->generatePlayDevCardActions(boardState->currentPlayer);
...
Action::PackedAction RandomPlayer::getDiscardAction() {
    ...
    uint8_t totalResources = Player::totalResources(
        boardState
    ...
    for (Resource r : {Resource::Lumber, Resource::Brick, Resource::Wool,
                       Resource::Grain, Resource::Ore}) {
        uint8_t count = Player::unpackResource(
            boardState->packedPlayers[static_cast<uint8_t>(
    ...
    std::shuffle(resourcePool.begin(), resourcePool.end(),
                 RandomDevice::get_rng());
    for (uint8_t i = 0; i < toDiscard; ++i) {
        action = Action::packResource(
            action, resourcePool[i],
            Action::unpackResource(action, resourcePool[i]) + 1);
    }
