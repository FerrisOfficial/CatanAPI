...
Action::PackedAction buildAction(ActionType type, PlayerId playerId,
...
static bool playerHasAdjacentRoad(const BoardState& board, PlayerId playerId,
                                  EdgeId edgeId) {
    ...
    EdgeId adjacentEdge1 =
        Node::unpackAdjacentEdge(board.nodes[adjacentNode1], 0);
    EdgeId adjacentEdge2 =
        Node::unpackAdjacentEdge(board.nodes[adjacentNode1], 1);
    EdgeId adjacentEdge3 =
        Node::unpackAdjacentEdge(board.nodes[adjacentNode1], 2);
    EdgeId adjacentEdge4 =
        Node::unpackAdjacentEdge(board.nodes[adjacentNode2], 0);
    EdgeId adjacentEdge5 =
        Node::unpackAdjacentEdge(board.nodes[adjacentNode2], 1);
    EdgeId adjacentEdge6 =
        Node::unpackAdjacentEdge(board.nodes[adjacentNode2], 2);

    for (EdgeId adjacentEdgeId :
         {adjacentEdge1, adjacentEdge2, adjacentEdge3, adjacentEdge4,
          adjacentEdge5, adjacentEdge6}) {
...
static bool playerHasAdjacentSettlementOrCity(const BoardState& board,
                                              PlayerId playerId,
                                              EdgeId edgeId) {
    auto edge = board.edges[edgeId];
    ...
    if (ownerA == playerId && (structureA == StructureType::Settlement ||
                               structureA == StructureType::City)) {
        return true;
    }

    StructureType structureB = Node::unpackStructure(board.nodes[nodeB]);
    PlayerId ownerB = Node::unpackOwner(board.nodes[nodeB]);
    if (ownerB == playerId && (structureB == StructureType::Settlement ||
                               structureB == StructureType::City)) {
        return true;
    }
...
static bool playerHasAvailableStructure(const BoardState& board,
                                        PlayerId playerId,
        ...
        if (Node::unpackStructure(nodes[nodeId]) ==
            ...
            for (int i = 0; i < 3; ++i) {
                EdgeId adjacentEdgeId =
                    Node::unpackAdjacentEdge(nodes[nodeId], i);
                ...
                if (adjacentEdgeId != EdgeIdNone) {
                    ...
                    NodeId adjacentNodeA =
                        Edge::unpackAdjacentNode(adjacentEdge, 0);
                    NodeId adjacentNodeB =
                        Edge::unpackAdjacentNode(adjacentEdge, 1);

                    for (NodeId adjacentNodeId :
                         {adjacentNodeA, adjacentNodeB}) {
                        if (adjacentNodeId != nodeId) {
                            StructureType structure =
                                Node::unpackStructure(nodes[adjacentNodeId]);
...
std::vector<Action::PackedAction> BoardState::generateTwoToOnePortTradeActions(
    ...
    for (NodeId nodeId : {15, 25}) {
        ...
        if (owner == playerId && (structure == StructureType::Settlement ||
                                  structure == StructureType::City)) {
            hasBrickPort = true;
        }
    }
...
static bool edgeIsValidForRoadBuildingDevCard(const BoardState& board,
                                              PlayerId playerId,
                                              EdgeId edgeId) {
...
// Zawiera duplikaty (budowanie dwóch dróg na tych samych krawędziach w różnej
// kolejności)
std::vector<Action::PackedAction>
BoardState::generatePlayDevCardRoadBuildingActions(PlayerId playerId) {
        ...
        if (availableRoads >= 1) {
            ...
            if (availableRoads == 1) {
                ...
                for (EdgeId firstEdge : firstCandidates) {
                    roadBuildingActions.push_back(
                        buildAction(ActionType::PlayDevCardRoadBuilding,
                                    playerId, firstEdge, EdgeIdNone));
                }
            } else {
                ...
                for (size_t i = 0; i < firstCandidates.size(); ++i) {
                    ...
                    // Temporarily modify board to account for first road being
                    // built
                    BoardState tempBoard = *this;
                    tempBoard.handleBuildRoad(
                        buildAction(ActionType::BuildRoad, playerId, firstEdge),
                        playerId);

                    for (EdgeId secondEdge = 0; secondEdge < EDGE_COUNT;
                         ++secondEdge) {
                        if (secondEdge != firstEdge &&
                            edgeIsValidForRoadBuildingDevCard(
                                tempBoard, playerId, secondEdge)) {
                            roadBuildingActions.push_back(
                                buildAction(ActionType::PlayDevCardRoadBuilding,
                                            playerId, firstEdge, secondEdge));
                        }
                    }
                }
            }
...
std::vector<Action::PackedAction>
BoardState::generatePlayDevCardYearOfPlentyActions(PlayerId playerId) {
    ...
    if (Player::unpackDevCard(player, DevType::YearOfPlenty) > 0) {
        for (Resource firstResource :
            ...
            for (Resource secondResource :
                 {Resource::Brick, Resource::Lumber, Resource::Wool,
                  Resource::Grain, Resource::Ore}) {
                yearOfPlentyActions.push_back(
                    buildAction(ActionType::PlayDevCardYearOfPlenty, playerId,
...
std::vector<Action::PackedAction>
BoardState::generatePlayDevCardMonopolyActions(PlayerId playerId) {
    ...
    if (Player::unpackDevCard(player, DevType::Monopoly) > 0) {
        for (Resource targetResource :
             {Resource::Brick, Resource::Lumber, Resource::Wool,
              Resource::Grain, Resource::Ore}) {
            monopolyActions.push_back(
                buildAction(ActionType::PlayDevCardMonopoly, playerId,
...
std::vector<Action::PackedAction> BoardState::generatePlaceInitialStructures(
    PlayerId playerId) {
...
std::vector<Action::PackedAction> BoardState::generatePlace2InitialStructures(
    PlayerId playerId) {
    ...
    for (NodeId nodeId = 0; nodeId < NODE_COUNT; ++nodeId) {
        ...
        for (EdgeId eId : adjEdges) {
            if (eId != EdgeIdNone && !Edge::unpackHasRoad(edges[eId])) {
                place2SettlementActions.push_back(
                    buildAction(ActionType::Place2InitialStructures, playerId,
                                nodeId, eId));
            }
        }
    }
...
std::vector<Action::PackedAction> BoardState::generatePlayDevCardActions(
    PlayerId playerId) {
    ...
    playDevCardActions.insert(playDevCardActions.end(), knightActions.begin(),
                              knightActions.end());
    playDevCardActions.insert(playDevCardActions.end(),
                              roadBuildingActions.begin(),
                              roadBuildingActions.end());
    playDevCardActions.insert(playDevCardActions.end(),
                              yearOfPlentyActions.begin(),
                              yearOfPlentyActions.end());
    playDevCardActions.insert(playDevCardActions.end(), monopolyActions.begin(),
                              monopolyActions.end());
...
std::vector<Action::PackedAction> BoardState::generateMoveRobberActions(
    PlayerId playerId) {
    std::vector<Action::PackedAction> moveRobberActions;

    for (HexId hexId = 0; hexId < HEX_COUNT; ++hexId) {
        if (hexId != robberPosition) {
            moveRobberActions.push_back(
                buildAction(ActionType::MoveRobber, playerId, hexId));
        }
    }
