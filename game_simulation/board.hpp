...
constexpr uint8_t unpackCatanNumber(PackedHex h) { return h & 0xF; }

// Resource packing/unpacking (bits 4-6)
constexpr PackedHex packResource(PackedHex h, Resource resource) {
    return (h & ~(0x7U << 4)) |
           (uint16_t(static_cast<uint8_t>(resource) & 0x7) << 4);
}
...
constexpr PackedHex packPlayerValue(PackedHex h, PlayerId player,
                                    uint8_t value) {
...
constexpr PackedHex makeHex(uint8_t catanNumber, Resource resource,
                            uint8_t player0Value = 0,
                            uint8_t player1Value = 0) {
    PackedHex h = 0;
...
constexpr PackedNode packOwner(PackedNode n, PlayerId owner) {
    return (n & ~(0x3ULL << 2)) |
           (uint64_t(static_cast<uint8_t>(owner) & 0x3) << 2);
}
...
constexpr PackedNode packAdjacentHex(PackedNode n, uint8_t hexIndex,
                                     HexId hexId) {
...
constexpr PackedNode packPortType(PackedNode n, PortType portType) {
    return (n & ~(0x7ULL << 19)) |
           (uint64_t(static_cast<uint8_t>(portType) & 0x7) << 19);
}
...
constexpr PackedNode packAdjacentEdge(PackedNode n, uint8_t edgeIndex,
                                      EdgeId edgeId) {
...
constexpr PackedNode makeNode(
    HexId hex1, HexId hex2, HexId hex3, EdgeId edge1, EdgeId edge2,
    EdgeId edge3, StructureType structure = StructureType::NoStructure,
    PlayerId owner = PlayerId::NoPlayer, PortType portType = PortType::NoPort) {
    PackedNode n = 0;
...
constexpr bool unpackHasRoad(PackedEdge e) { return e & 1; }

// Owner packing/unpacking (bits 1-2)
constexpr PackedEdge packOwner(PackedEdge e, PlayerId owner) {
    return (e & ~(0x3U << 1)) |
           (uint32_t(static_cast<uint8_t>(owner) & 0x3) << 1);
}
...
constexpr PackedEdge packAdjacentNode(PackedEdge e, uint8_t nodeIndex,
                                      NodeId nodeId) {
...
constexpr PackedEdge makeEdge(NodeId node1, NodeId node2, bool hasRoad = false,
...
struct BoardState {
    ...
    Player::PackedPlayer packedPlayers[2] = {Player::makeNewPlayer(),
    ...
    void handlePlaceInitialStructures(Action::PackedAction action,
                                      PlayerId playerId);
    void handlePlace2InitialStructures(Action::PackedAction action,
                                       PlayerId playerId);
    Action::PackedAction handleEndTurn();
    Action::PackedAction handleRollDice(Action::PackedAction action);
    Action::PackedAction handleMoveRobber(Action::PackedAction action,
                                          PlayerId playerId);
    ...
    Action::PackedAction handlePlayDevCardKnight(Action::PackedAction action,
                                                 PlayerId playerId);
    void handlePlayDevCardRoadBuilding(Action::PackedAction action,
                                       PlayerId playerId);
    void handlePlayDevCardYearOfPlenty(Action::PackedAction action,
                                       PlayerId playerId);
    Action::PackedAction handlePlayDevCardMonopoly(Action::PackedAction action,
                                                   PlayerId playerId);
    Action::PackedAction handleStealResource(Action::PackedAction action,
                                             PlayerId playerId);
    ...
    void handleUndoStealResource(Action::PackedAction action,
                                 PlayerId playerId);
    void handleUndoDiscardResources(Action::PackedAction action,
                                    PlayerId playerId);
    void handleUndoBuildRoad(Action::PackedAction action, PlayerId playerId);
    void handleUndoBuildSettlement(Action::PackedAction action,
                                   PlayerId playerId);
    ...
    void handleUndoPlayDevCardKnight(Action::PackedAction action,
                                     PlayerId playerId);
    void handleUndoPlayDevCardRoadBuilding(Action::PackedAction action,
                                           PlayerId playerId);
    void handleUndoPlayDevCardYearOfPlenty(Action::PackedAction action,
                                           PlayerId playerId);
    void handleUndoPlayDevCardMonopoly(Action::PackedAction action,
                                       PlayerId playerId);
    void handleUndoTradeBank(Action::PackedAction action, PlayerId playerId);
    void handleUndoReceiveResources(Action::PackedAction action,
                                    PlayerId playerId);
    void handleUndoPlaceInitialSettlement(Action::PackedAction action,
                                          PlayerId playerId);
    void handleUndoPlace2InitialSettlement(Action::PackedAction action,
                                           PlayerId playerId);
    ...
    std::vector<Action::PackedAction> generateBuildSettlementActions(
        PlayerId playerId);
    std::vector<Action::PackedAction> generateBuildCityActions(
        PlayerId playerId);
    ...
    std::vector<Action::PackedAction> generateThreeToOnePortTradeActions(
        PlayerId playerId);
    std::vector<Action::PackedAction> generateBuyDevCardActions(
        PlayerId playerId);
    std::vector<Action::PackedAction> generatePlayDevCardActions(
        PlayerId playerId);
    std::vector<Action::PackedAction> generatePlayDevCardKnightActions(
        PlayerId playerId);
    std::vector<Action::PackedAction> generatePlayDevCardRoadBuildingActions(
        PlayerId playerId);
    std::vector<Action::PackedAction> generatePlayDevCardYearOfPlentyActions(
        PlayerId playerId);
    std::vector<Action::PackedAction> generatePlayDevCardMonopolyActions(
        PlayerId playerId);
    std::vector<Action::PackedAction> generatePlaceInitialStructures(
        PlayerId playerId);
    std::vector<Action::PackedAction> generatePlace2InitialStructures(
        PlayerId playerId);
    std::vector<Action::PackedAction> generateMoveRobberActions(
        PlayerId playerId);
};
