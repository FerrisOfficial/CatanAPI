...
const char* structureTypeName(StructureType s) {
    switch (s) {
        ...
        case StructureType::City:
            return "City";
        case StructureType::NoStructure:
            return "NoStructure";
    }
...
Dumper::Dumper(std::filesystem::path logsDir) {
    ...
    auto filename =
        std::string("game_") + timestampForFilenameLocal() + ".jsonl";
    auto outPath = logsDir / filename;

    outPath_ = outPath.string();
    out_.open(outPath_, std::ios::out | std::ios::binary);
    if (!out_.is_open()) {
        throw std::runtime_error("Dumper: failed to open log file: " +
                                 outPath_);
    }
...
const std::string& Dumper::filePath() const { return outPath_; }
...
void Dumper::recordPlayersInfo() {
    std::ostringstream oss;
    oss << "{\"seq\":" << nextSeq() << ",\"event\":\"players\",\"timestamp\":\""
        << escapeJson(nowIso8601Local()) << "\",\"players\":["
...
void Dumper::recordStart() {
    std::ostringstream oss;
    oss << "{\"seq\":" << nextSeq()
        << ",\"event\":\"start\",\"format_version\":1,\"timestamp\":\""
        << escapeJson(nowIso8601Local()) << "\",\"file\":\""
        << escapeJson(outPath_) << "\"}";
    writeJsonLine(oss.str());
}
...
void Dumper::recordTurnStart(const Board::BoardState& state) {
    std::ostringstream oss;
    oss << "{\"seq\":" << nextSeq()
        << ",\"event\":\"turn_start\",\"timestamp\":\""
        << escapeJson(nowIso8601Local())
...
void Dumper::recordInitialState(const Board::BoardState& state) {
    std::ostringstream oss;
    oss << "{\"seq\":" << nextSeq()
        << ",\"event\":\"initial_state\",\"timestamp\":\""
        << escapeJson(nowIso8601Local())
        << "\",\"state\":" << stateToJson(state) << '}';
    writeJsonLine(oss.str());
}

void Dumper::recordDiceRoll(uint8_t diceNumber,
                            const Board::BoardState& state) {
    std::ostringstream oss;
    oss << "{\"seq\":" << nextSeq() << ",\"event\":\"dice\",\"timestamp\":\""
        << escapeJson(nowIso8601Local())
...
void Dumper::recordActionApplied(Action::PackedAction action,
                                 const Board::BoardState& state,
                                 const std::string& phase) {
    std::ostringstream oss;
    oss << "{\"seq\":" << nextSeq()
        << ",\"event\":\"action_applied\",\"timestamp\":\""
        << escapeJson(nowIso8601Local()) << "\",\"phase\":\""
        << escapeJson(phase) << "\",\"action\":" << actionToJson(action)
        << ",\"state\":" << stateToJson(state) << '}';
    writeJsonLine(oss.str());
}
...
uint64_t Dumper::nextSeq() { return seq_++; }
...
std::string Dumper::actionToJson(Action::PackedAction action) {
    ...
    oss << '{' << "\"raw\":" << static_cast<unsigned long long>(action)
        << ",\"type\":" << static_cast<unsigned>(type) << ",\"type_name\":\""
        << escapeJson(actionTypeName(type)) << "\""
        ...
        << static_cast<unsigned>(
               Action::unpackResource(action, Resource::Brick))
        << ",\"lumber\":"
        << static_cast<unsigned>(
               Action::unpackResource(action, Resource::Lumber))
        << ",\"wool\":"
        << static_cast<unsigned>(Action::unpackResource(action, Resource::Wool))
        << ",\"grain\":"
        << static_cast<unsigned>(
               Action::unpackResource(action, Resource::Grain))
        << ",\"ore\":"
        << static_cast<unsigned>(Action::unpackResource(action, Resource::Ore))
        << "}}";
...
std::string Dumper::stateToJson(const Board::BoardState& state) const {
    ...
    oss << '{' << "\"robber\":" << static_cast<unsigned>(state.robberPosition)
        << ",\"current_player\":" << static_cast<unsigned>(state.currentPlayer)
        << ",\"current_player_name\":\""
        << escapeJson(playerNameForId(state.currentPlayer)) << "\""
        << ",\"turn\":" << static_cast<unsigned>(state.currentTurn)
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        oss << '{' << "\"id\":" << i << ",\"name\":\""
            << escapeJson(i == 0 ? playerNames_[0] : playerNames_[1]) << "\""
            << ",\"packed\":" << static_cast<unsigned long long>(p)
            << ",\"victory_points\":"
            << static_cast<unsigned>(Player::unpackVictoryPoints(p))
            << ",\"used_knights\":"
            << static_cast<unsigned>(Player::unpackUsedKnights(p))
            << ",\"longest_road_length\":"
            << static_cast<unsigned>(Player::unpackLongestRoadLength(p))
            << ",\"longest_road_flag\":"
            << (Player::unpackLongestRoadFlag(p) ? "true" : "false")
            << ",\"largest_army_flag\":"
            << (Player::unpackLargestArmyFlag(p) ? "true" : "false")
            << ",\"resources\":{"
            << "\"brick\":"
            << static_cast<unsigned>(Player::unpackResource(p, Resource::Brick))
            << ",\"lumber\":"
            << static_cast<unsigned>(
                   Player::unpackResource(p, Resource::Lumber))
            << ",\"wool\":"
            << static_cast<unsigned>(Player::unpackResource(p, Resource::Wool))
            << ",\"grain\":"
            << static_cast<unsigned>(Player::unpackResource(p, Resource::Grain))
            << ",\"ore\":"
            << static_cast<unsigned>(Player::unpackResource(p, Resource::Ore))
            << ",\"total\":" << static_cast<unsigned>(Player::totalResources(p))
            << "}"
            << ",\"dev_cards\":{"
            << "\"knight\":"
            << static_cast<unsigned>(Player::unpackDevCard(p, DevType::Knight))
            << ",\"road_building\":"
            << static_cast<unsigned>(
                   Player::unpackDevCard(p, DevType::RoadBuilding))
            << ",\"year_of_plenty\":"
            << static_cast<unsigned>(
                   Player::unpackDevCard(p, DevType::YearOfPlenty))
            << ",\"monopoly\":"
            << static_cast<unsigned>(
                   Player::unpackDevCard(p, DevType::Monopoly))
            << ",\"victory_point\":"
            << static_cast<unsigned>(
                   Player::unpackDevCard(p, DevType::VictoryPoint))
            << ",\"total\":" << static_cast<unsigned>(Player::totalDevCards(p))
            << "}"
            << ",\"available_structures\":{"
            << "\"roads\":"
            << static_cast<unsigned>(
                   Player::unpackAvailableStructures(p, StructureType::Road))
            << ",\"settlements\":"
            << static_cast<unsigned>(Player::unpackAvailableStructures(
                   p, StructureType::Settlement))
            << ",\"cities\":"
            << static_cast<unsigned>(
                   Player::unpackAvailableStructures(p, StructureType::City))
            << "}" << '}';
    }
    oss << ']';
    ...
    oss << ",\"packed_bank\":" << static_cast<unsigned long long>(pb)
        ...
        << "\"resources\":{"
        << "\"brick\":"
        << static_cast<unsigned>(Bank::unpackResource(pb, Resource::Brick))
        << ",\"lumber\":"
        << static_cast<unsigned>(Bank::unpackResource(pb, Resource::Lumber))
        << ",\"wool\":"
        << static_cast<unsigned>(Bank::unpackResource(pb, Resource::Wool))
        << ",\"grain\":"
        << static_cast<unsigned>(Bank::unpackResource(pb, Resource::Grain))
        << ",\"ore\":"
        << static_cast<unsigned>(Bank::unpackResource(pb, Resource::Ore)) << "}"
        << ",\"dev_cards\":{"
        << "\"knight\":"
        << static_cast<unsigned>(Bank::unpackDevCard(pb, DevType::Knight))
        << ",\"road_building\":"
        << static_cast<unsigned>(Bank::unpackDevCard(pb, DevType::RoadBuilding))
        << ",\"year_of_plenty\":"
        << static_cast<unsigned>(Bank::unpackDevCard(pb, DevType::YearOfPlenty))
        << ",\"monopoly\":"
        << static_cast<unsigned>(Bank::unpackDevCard(pb, DevType::Monopoly))
        << ",\"victory_point\":"
        << static_cast<unsigned>(Bank::unpackDevCard(pb, DevType::VictoryPoint))
        << ",\"total_cached\":"
        << static_cast<unsigned>(Bank::unpackTotalDevCount(pb))
        << ",\"total_computed\":"
        << static_cast<unsigned>(Bank::computeTotalDevCards(pb)) << "}" << '}';
    ...
    for (int i = 0; i < HEX_COUNT; ++i) {
        ...
        oss << '{' << "\"id\":" << i
            ...
            << ",\"catan_number\":"
            << static_cast<unsigned>(Board::Hex::unpackCatanNumber(h))
            << ",\"resource\":" << static_cast<unsigned>(res)
            << ",\"resource_name\":\"" << escapeJson(resourceName(res)) << "\""
            << ",\"player_values\":{"
            << "\"player0\":"
            << static_cast<unsigned>(
                   Board::Hex::unpackPlayerValue(h, PlayerId::Player0))
            << ",\"player1\":"
            << static_cast<unsigned>(
                   Board::Hex::unpackPlayerValue(h, PlayerId::Player1))
            << "}" << '}';
    }
    oss << ']';
    ...
    for (int i = 0; i < NODE_COUNT; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            if (j) oss << ',';
            const auto hid =
                Board::Node::unpackAdjacentHex(n, static_cast<uint8_t>(j));
            if (hid == HexIdNone) {
                oss << "null";
            } else {
        ...
        for (int j = 0; j < 3; ++j) {
            if (j) oss << ',';
            const auto eid =
                Board::Node::unpackAdjacentEdge(n, static_cast<uint8_t>(j));
            if (eid == EdgeIdNone) {
                oss << "null";
            } else {
    ...
    for (int i = 0; i < EDGE_COUNT; ++i) {
        ...
        oss << '{' << "\"id\":" << i
            << ",\"packed\":" << static_cast<unsigned>(e) << ",\"has_road\":"
            << (Board::Edge::unpackHasRoad(e) ? "true" : "false")
            << ",\"owner\":" << static_cast<unsigned>(owner)
            << ",\"owner_name\":\"" << escapeJson(playerIdName(owner)) << "\""
            << ",\"adjacent_nodes\":[";
